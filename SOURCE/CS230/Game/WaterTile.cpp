...
WaterTile::WaterTile(Rect rect, int row, int column) : Tile(rect, row, column), timer(0.35) {}

void WaterTile::Draw(TransformMatrix cameraMatrix)
{

	objectMatrix = TranslateMatrix{ {rect.bottomLeft} };
	sprite.Draw(cameraMatrix * objectMatrix * ScaleMatrix{ {0.3, 0.32986} });

}



void WaterTile::Update([[maybe_unused]] double dt)
{
	sprite.Update(dt);
	...
	for (int i = 0; i < World::GetObjects().GetGameObjectStorage().size(); i++)
	{
		...
		if (dynamic_cast<LavaSlime*>(target))
		{
			...
			if (IsCollision(target->GetPosition()) == true)
			{
				...
				{
					slimeinteract = true;
					sprite.PlayAnimation(static_cast<int>(2));
					timer.Init();
...
void WaterTile::Load()
{
	sprite.Load("Assets/ScriptSprites/WaterTile.spt");
	sprite.PlayAnimation(static_cast<int>(1));
}
